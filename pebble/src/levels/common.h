#pragma once
#include <pebble.h>
#include "../game.h"
#include "../sprite.h"

void moveToExit(uint16_t* _state);
void addCluter(int _x, int _yUp, int _yDn);
void enterRoom(uint16_t* _state);
uint16_t randomiseChoices(int8_t* _choices, int _stage);
void stonesCommon(uint16_t* _state, int8_t* _fire, int8_t* _correct);
void shuffler(int8_t* _choices, int _offset, int _rand);
void renderStonesCommonFire(GContext* _ctx, int8_t* _correct, int8_t _fire, int16_t _state);

char* txtEvilWind0(void);
char* txtEvilWind1(void);
char* txtEvilBell0(void);
char* txtEvilBell1(void);
char* txtGoodFaerie0(void);
char* txtGoodFaerie1(void);
char* txtShortcut0(void);
char* txtShortcut1(void);
char* txtQuietVoice0(void);
char* txtQuietVoice1(void);
