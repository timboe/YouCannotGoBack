...
static uint8_t s_maze0[3][3] = {{0},{0},{0}};
static uint8_t s_maze1[5][7] = {{0},{0},{0},{0},{0}};

static uint8_t s_stack[5*7*2] = {0};
static uint16_t s_head = 0;

static Options_t s_mazeDisp0[3][3] = {{kN},{kN},{kN}};
static Options_t s_mazeDisp1[5][7] = {{kN},{kN},{kN},{kN},{kN}};

void updateProcArrows(GContext* _ctx) {
  renderFloor(_ctx, 0);
  renderStandingStone(_ctx, 5, 6 + s_start*4, GColorBlack); //, kCircle);
  renderClutter(_ctx);
  renderPlayer(_ctx);
  renderFloorArrows(_ctx, s_mazeDisp0, s_maze0, s_mazeDisp1, s_maze1, s_tickB);
  // Draw on top
  if (m_player.m_position.x <= SIZE*4 && getFlash(true)) renderPlayer(_ctx);
...
bool tickArrows(bool _doInit) {
  ...
  if (s_state == 0) { // start initial move
    ...
    switch (s_start) {
      case 0: m_player.m_target = GPoint(SIZE*4, SIZE*5); break;
      case 1: m_player.m_target = GPoint(SIZE*4, SIZE*9); break;
      case 2: m_player.m_target = GPoint(SIZE*4, SIZE*13); break;
    }
