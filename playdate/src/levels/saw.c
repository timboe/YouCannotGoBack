...
bool tickSaw(bool _doInit) {
  ...
  if (s_state == 0) { // start initial move
    ...
    m_player_ycgb.m_target_x = 4*SIZE;
    m_player_ycgb.m_target_y = (6 + (2*getPlayerChoice())) * SIZE;
    ...
    if (s_count < 4 + m_dungeon.m_difficulty) {
      ...
      if (abs( s_position+35 - m_player_ycgb.m_position_x ) < 30) {
        if (s_type[s_count] == 1 && m_player_ycgb.m_position_y < 9*SIZE) s_state = 2;
        else if (s_type[s_count] == 2 && (m_player_ycgb.m_position_y < 7*SIZE || m_player_ycgb.m_position_y > 9*SIZE)) s_state = 2;
        else if (s_type[s_count] == 3 && m_player_ycgb.m_position_y > 7*SIZE ) s_state = 2;
      }
    } else {
      setGameState(kLevelSpecific);
      sawSound(false);
      m_player_ycgb.m_position_x += (PLAYER_SPEED * 2);
      if (m_player_ycgb.m_position_x > 20*SIZE) {
    ...
    sawVolume( 1.0f - (abs(s_position - m_player_ycgb.m_position_x) / SAW_SOUND_DISTANCE) );
  } else if (s_state == 2) {
