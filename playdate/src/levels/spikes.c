...
bool tickSpikes(bool _doInit) {
  ...
  if (s_state == 0) { // start initial move
    ...
    for (int _i = 0; _i < 3; ++_i) {
      int _x = abs((SIZE*5) + (SIZE*4*_i) - m_player_ycgb.m_position_x);
    ...
    if (s_state % 2 == 1) {
      ...
      switch (s_state) {
        ...
        case 5: if (m_player_ycgb.m_position_x > SIZE*11 - SIZE/2) ++s_state; break;
        case 7: if (m_player_ycgb.m_position_x > SIZE*14 - SIZE/2) s_state += 2; break; // Don't want to wait for user-input here
        case 9: if (m_player_ycgb.m_position_x > SIZE*17) ++s_state; break;
      }
    }
